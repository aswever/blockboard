...
const atomFromCookie = <T>(key: string, initialValue?: string): WritableAtom<T, unknown, void> => {
...
export const useSignedToken = (init?: string) => {
  if (!signedTokenAtom) signedTokenAtom = atomFromCookie<SignedToken | null>("signedToken", init);
